...
class EmailService:
	...
	def send_email_reset(self, supplied_email, reset=True):
		...
		subject = 'Reset Password' if reset else "Your invitation to Market(r)"

		msg = Message(subject, sender=self.sender, recipients=[supplied_email])
		link = url_for('update_password', token=token, _external=True)
		msg.body = f"Your password reset link is: {link}" if reset else f"Claim your account here: {link}"
		self.mail.send(msg)
	...
	def update_password(self, supplied_password, token=None):
		...
		tup = (password, email)
		initial_check = "SELECT TOP 1 email FROM dbo.customer_basic WHERE email = ?"
		data, cursor = db.execute(initial_check, True, (email,))
		data = cursor.fetchone()
		if data:
			query = "UPDATE dbo.customer_basic SET password = ? WHERE email = ?"
		else:
			query = "UPDATE dbo.secondary_users SET password = ? WHERE email = ?"
